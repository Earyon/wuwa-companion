...
function pwaStatusHTML(){return `<p>${pwaState.waiting?tr('Mise à jour prête. Enregistre tes modifications puis ouvre la page de mise à jour.','Update ready. Save your changes, then open the update page.'):pwaState.error?tr('Vérification indisponible. La version déjà installée reste utilisable.','Update check unavailable. The installed version remains usable.'):pwaState.registration?.active?tr('Application installée pour le hors ligne.','Application installed for offline use.'):tr('Installation du mode hors ligne en attente ou indisponible dans ce navigateur.','Offline installation is pending or unavailable in this browser.')}</p><a class="companion-button" href="./mise-a-jour.html" target="_blank" rel="noopener">${tr('Ouvrir la mise à jour','Open the update page')}</a>`;}
