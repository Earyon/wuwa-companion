...
function settingsPage(){
 ...
 <p>${tr('Sauvegarde personnelle au format JSON, pour restaurer ou transférer ton compte. Les catalogues du jeu et les historiques d’invocations ne sont pas inclus. Exporte les historiques depuis Plus → Invocations.','Personal JSON backup to restore or transfer your account. Game catalogues and pull histories are not included. Export histories from More → Convenes.')}</p>
 ...
 companionPanel(tr('Sources & mises à jour','Sources & updates'),`<p>${esc(catalogState.source||'Encore')} · ${esc(catalogState.gameVersion||'?')}<br>${DATA.length} ${tr('Résonateurs','Resonators')} · ${WEAPONS.length} ${tr('armes','weapons')}</p><p><a href="https://www.encore.moe/about" target="_blank" rel="noopener">Encore / WW_Data</a> · <a href="https://www.prydwen.gg/wuthering-waves/" target="_blank" rel="noopener">Prydwen</a> · <a href="https://game8.co/games/Wuthering-Waves" target="_blank" rel="noopener">Game8</a></p><p class="companion-note">${tr('Outil communautaire non officiel. Les noms et visuels du jeu appartiennent à leurs détenteurs. Les recommandations présentent leur source et leur date de vérification.','Unofficial community tool. Game names and artwork belong to their owners. Recommendations show their source and review date.')}</p>`);
}
