...
let resourceCatalogPending=false;
function refreshResourceCatalog(){
 if(currentView!=='account'||accountTab!=='resources')return;
 if(document.activeElement?.closest('#resourceSearch,#resourcesForm')){resourceCatalogPending=true;return;}
 resourceCatalogPending=false;render();
}
document.addEventListener('catalogue-ready',e=>{if(e.detail==='item')refreshResourceCatalog();});
document.addEventListener('focusout',()=>{if(resourceCatalogPending)queueMicrotask(refreshResourceCatalog);});
function resourcesInventory(){
 ...
 for(const [id,name]of [['3','Astrites'],['50002',tr('Marées radieuses','Radiant Tides')],['50005',tr('Marées de forge','Forging Tides')]])if(!catalog.has(id))catalog.set(id,{id,name,type:'',image:''});
 ...
 const filtered=[...catalog.values()].filter(r=>(gameLabel(r)+' '+r.name+' '+gameText(r.type)+' '+r.id).toLocaleLowerCase().includes(resourceQuery.toLocaleLowerCase())&&(resourceFilter==='all'||resourceFilter==='currency'&&['3','50002','50005'].includes(r.id)||resourceFilter==='known'&&state.resources[r.id]!=null||resourceFilter==='needed'&&needed.has(r.id)));
 ...
 <form id="resourceSearch" class="companion-form"><label>${tr('Rechercher','Search')}<input name="query" type="search" value="${esc(resourceQuery)}"></label><label>${tr('Afficher','Show')}<select name="filter">${[['all',tr('Toutes les ressources','All resources')],['currency',tr('Monnaies d’invocation','Convene currencies')],['known',tr('Stocks renseignés','Recorded stock')],['needed',tr('Besoin de l’objectif actif','Active goal needs')]].map(([v,n])=>`<option value="${v}" ${resourceFilter===v?'selected':''}>${n}</option>`).join('')}</select></label><button class="companion-button">${tr('Rechercher','Search')}</button></form>
 <form id="resourcesForm"><div class="companion-list">${rows.map(row=>inventoryCard(row,`<b>${esc(gameLabel(row))}</b><small>${esc(gameText(row.type))}</small><label class="companion-form"><input aria-label="${esc(row.name)}" name="item:${row.id}" type="number" min="0" max="1000000000000" step="1" placeholder="?" value="${Object.hasOwn(resourceDraft,row.id)?resourceDraft[row.id]??'':state.resources[row.id]??''}"></label>`)).join('')||`<p>${extendedCatalog.loading.item?tr('Chargement…','Loading…'):tr('Aucune ressource dans cette sélection.','No resources in this selection.')}</p>`}</div>
