...
function resourcesInventory(){
 ...
 const filtered=[...catalog.values()].filter(r=>(r.name+' '+r.type+' '+r.id).toLocaleLowerCase().includes(resourceQuery.toLocaleLowerCase())&&(resourceFilter==='all'||resourceFilter==='currency'&&['3','50002','50005'].includes(r.id)||resourceFilter==='known'&&state.resources[r.id]!=null||resourceFilter==='needed'&&needed.has(r.id)));
