...
function drawTalentEditor(state){
 ...
 requestAnimationFrame(()=>{if(detail!==document.getElementById('talentDetail'))return;fitTalentBoard();detail.scrollTop=scrollTop;const text=detail.querySelector('.source-description');if(text)text.scrollTop=descriptionScroll;});
...
document.getElementById('skillsEditor').addEventListener('click',event=>{
 ...
 if(node){talentSelection=node.dataset.talent;drawTalentEditor();if(!document.querySelector('.talent-board').getClientRects().length)document.querySelector('#talentDetail h3').focus({preventScroll:true});return;}
