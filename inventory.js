...
function normalizeExtended(kind,payload){
 ...
 const normalized=rows.map(row=>{
  ...
  seen.add(row.Id);
  return {id:String(row.Id),kind,name:content(row.Name),image:assetUrl(row.IconSmall||row.Icon,{local:false}),type:content(row.TypeName||row.Type),sets:(row.FetterGroups||[]).map(set=>({id:String(set.Id),name:content(set.Name)}))};
 });
...
function extendedArtwork(rows){return rows.map(row=>({...row,image:assetUrl(row.image)}));}
async function loadExtended(kind,{refresh=false}={}){
 ...
 try{
  ...
  if(!refresh&&cached?.gameVersion===catalogState.gameVersion){try{extendedCatalog[kind]=extendedArtwork(normalizeExtended(kind,cached.payload));return;}catch{/* Fetch a valid replacement. */}}
  let payload;
  if(!refresh&&catalogState.gameVersion===frenchGameText?.gameVersion){
   try{const bundled=await fetchJSON('./data/'+kind+'-catalogue.json');if(bundled.schema!==1||bundled.gameVersion!==catalogState.gameVersion)throw Error('Invalid bundled inventory');normalizeExtended(kind,bundled.payload);payload=bundled.payload;}catch{/* The live source remains available if the bundled file is unusable. */}
  }
  if(!payload)payload=await fetchJSON(`${ENCORE_BASE}/en/${kind}`);
  const rows=normalizeExtended(kind,payload);extendedCatalog[kind]=extendedArtwork(rows);
  ...
  try{const old=JSON.parse(localStorage.getItem(key)||'null');if(old?.payload)extendedCatalog[kind]=extendedArtwork(normalizeExtended(kind,old.payload));}catch(e){/* Preserve invalid cache for diagnosis. */}
 }finally{extendedCatalog.loading[kind]=false;document.dispatchEvent(new CustomEvent('catalogue-ready',{detail:kind}));}
}
function catalogOptions(rows,id){return `<datalist id="${id}">${rows.map(row=>`<option value="${esc(gameLabel(row))}"></option>`).join('')}</datalist>`;}
...
function personalInventory(){
 ...
 if(accountTab==='weap'){
  ...
  const filtered=data.weapons.filter(w=>(gameLabel(WEAPONS.find(x=>x.id===w.catalogId))||gameText(w.name||w.catalogId)).toLowerCase().includes(inventoryQuery.toLowerCase()));
  ...
  <form id="weaponInventoryForm" class="companion-form"><label>${tr('Arme','Weapon')}<input name="weapon" list="weaponCatalogue" required value="${esc((source?gameLabel(source):'')||edit?.name||edit?.catalogId||'')}" autocomplete="off"></label>${catalogOptions(WEAPONS,'weaponCatalogue')}
  ...
  ${edit?companionButton('cancel-inventory',tr('Annuler','Cancel')):''}${inventorySearch()}<div class="companion-list">${filtered.map(w=>{const row=WEAPONS.find(x=>x.id===w.catalogId),owner=CompanionStore.weaponOwner(data,w.id);return inventoryCard(row,`<b>${esc(row?gameLabel(row):gameText(w.name||w.catalogId))} · #${data.weapons.indexOf(w)+1}</b><small>${tr('Niveau','Level')} ${w.level??'?'} · R${w.rank??'?'} · ${esc(row?weaponLabel(row.type):'?')}<br>${owner?esc(gameLabel(DATA.find(c=>c.id===owner))||owner):tr('Non équipée','Unequipped')}</small>`,companionButton('edit-weapon',tr('Modifier','Edit'),w.id)+companionButton('remove-weapon',tr('Retirer','Remove'),w.id)+(owner&&DATA.some(c=>c.id===owner)?companionButton('edit-weapon-owner',tr('Voir le Résonateur','View Resonator'),owner):''));}).join('')||`<p>${tr('Aucun exemplaire enregistré.','No copies recorded.')}</p>`}</div>`);
 }
...
Object.assign(companionActions,{
 ...
 'remove-weapon':id=>{
  ...
  const name=gameLabel(DATA.find(c=>c.id===owner))||owner;
...
document.querySelector('#view').addEventListener('submit',event=>{
 ...
 try{
  ...
  if(form.id==='weaponInventoryForm'){
   const row=WEAPONS.find(w=>(gameLabel(w)===values.get('weapon')||w.name===values.get('weapon')));const original=inventoryEdit?inventoryOriginal:null;
