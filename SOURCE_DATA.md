...
# Complément de menus du 14 septembre 2026

Source locale 3.6.13 : les lecteurs de tables générés ont décodé 96 lignes `rolepropertygrowth`, 192 `weaponpropertygrowth`, 1 945 `skilldescription`, 645 `weaponreson` et 38 `roletag`. Leurs empreintes rejoignent `game-source.json`. Les projections regroupent les multiplicateurs par compétence/niveau et les descriptions par rang d'arme, en remplaçant les paramètres des textes FR/EN officiels. Les 58 références se régénèrent à l'identique ; 122 armes et les nouvelles règles également.

L'inspection de `RoleAttributeTabView.Uho/UpdateAttribute` montre que le menu ne lit pas `RoleInfo.ShowProperty`. Il utilise `CommonParam.RoleAttributeDisplay6` : `[2,7,10,11,8,9]`. La lecture SQLite de cette entrée, 28 octets (longueur puis six entiers), confirme les six identifiants. La référence antérieure pointait sur 13, un champ obsolète ; les projections utilisent désormais la liste effective. Les courbes restent indexées sur **niveau et ascension** : sans ascension, un niveau frontière ambigu reste inconnu. Les statistiques présentées sont explicitement celles de base, sans prétendre simuler les bonus conditionnels ni synchroniser le compte Kuro.

Quarante et une icônes supplémentaires ont été décodées : attributs, styles de combat et étoile d’ascension, avec pixels/cadrages sources et WebP sans perte. Ajout total : 68238 octets avant déduplication. Les empreintes des **2 889 images officielles** passent le contrôle. Aucune miniature n'a été générée.

La faisabilité 3D a été testée avec le modèle local Hiyuki `R2T1TaidaonvMd10011`. L'identité, le squelette et les matériaux sont lisibles ; les données de rendu des LOD sont incomplètes dans le parseur utilisé. Deux exports glTF réels (avec matériaux, puis géométrie seule sans morphs) échouent dans `SkeletalMeshDto.ParseMeshRenderData`. Aucun modèle défectueux ni bibliothèque de rendu supplémentaire n'est intégré. Les rapports privés restent dans `test-results/local-game-audit/`. Référence de l'outil : [CUE4Parse](https://github.com/FabianFG/CUE4Parse), version fournie par [FModel août 2026](https://github.com/4sval/FModel/releases/tag/aug-2026).

`FontMedium` référence `LaguSansBold` pour les langues occidentales. Sa [licence web distincte](https://www.myfonts.com/collections/lagu-sans-font-alessio-laiso-type?tab=licensing) n'est pas acquise dans ce projet. La police n'est ni extraite pour publication ni achetée ; la police système est conservée. Ces références documentent une limite, sans affirmer que citer Kuro accorderait des droits de redistribution.
