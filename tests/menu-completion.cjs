...
(async()=>{
 ...
 try{
  for(const language of ['fr','en']){
   ...
   await page.locator('[data-echo-action="slot"][data-value="1"]').click();await page.locator('[data-echo-action="pick"]').click();await page.locator('#echoPickerCost').selectOption('4');assert.equal(await page.locator('.echo-choice').count(),0);await page.locator('#echoPickerCost').selectOption('1');assert.equal(await page.locator('.echo-choice').count(),2);await page.keyboard.press('Escape');
   for(const [width,height]of [[1724,1080],[1152,800],[720,1122],[720,450],[360,640]]){
    ...
    for(const section of ['overview','weapon','echo','forte','sequence']){
     ...
     if(section==='echo'&&width>=680){assert.equal(await page.locator('.echo-slot').evaluateAll(nodes=>{const stage=document.getElementById('editorStage').getBoundingClientRect();return nodes.every(e=>{const r=e.getBoundingClientRect();return r.top>=stage.top&&r.bottom<=stage.bottom+1&&r.width>=44&&r.height>=44;});}),true,'All five touch targets visible in the equipped rail');}
