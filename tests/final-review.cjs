...
(async()=>{
 ...
 try{
  for(const language of ['fr','en']){
   ...
   await p.evaluate(()=>{profileSources.clear();localStorage.setItem('wwc_profile_detail_v3:en:character:resonator:1413',JSON.stringify({version:'test',data:{Id:1413,Skills:{broken:true},Properties:[]}}));});
   ...
   await p.evaluate(()=>{const original=File.prototype.text;File.prototype.text=async function(){if(this.name==='slow.json'){window.__slowFinished=false;await new Promise(r=>{window.__releaseFile=r;});const result=await original.call(this);setTimeout(()=>{window.__slowFinished=true;},0);return result;}return original.call(this);};});
   const profile=value=>Buffer.from(JSON.stringify({id:'TEST',items:[{id:3,value}],achievements:[],todos:[]}));
   await p.locator('#trackerFile').setInputFiles({name:'slow.json',mimeType:'application/json',buffer:profile(10)});await p.locator('#trackerFile').setInputFiles({name:'fast.json',mimeType:'application/json',buffer:profile(20)});await p.waitForFunction(()=>trackerPending?.data.resources['3']===20);await p.evaluate(()=>window.__releaseFile());await p.waitForFunction(()=>window.__slowFinished);assert.equal(await p.evaluate(()=>trackerPending.data.resources['3']),20);
  ...
  const ctx=await browser.newContext({serviceWorkers:'block'}),p=await ctx.newPage();await p.route('https://**/*',r=>r.abort());await p.route('**/data/catalogue.json',r=>r.abort());await p.goto(base);await p.locator('[data-action="retry-start"]').waitFor();await p.locator('[data-action="more-tab"]').click();assert.equal(await p.locator('[data-action="export"]').count(),1);await p.locator('[data-action="check-update"]').click();assert.match(await p.locator('#pwaStatus').innerText(),/indisponible/);await ctx.close();
