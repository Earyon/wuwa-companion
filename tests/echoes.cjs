...
(async()=>{
 ...
 try{
  for(const language of ['fr','en']){
   ...
   await page.addInitScript(({chars,weapons,language})=>{if(localStorage.getItem('test-seeded'))return;localStorage.setItem('test-seeded','1');localStorage.setItem('wwc_lang',language);localStorage.setItem('wwc_owned_ids','["resonator:1","resonator:2"]');localStorage.setItem('wwc_account_data','{}');localStorage.setItem('wwc_catalog_canonical_v050',JSON.stringify({characters:chars,weapons,state:{gameVersion:'test',resourceVersion:'test'}}));localStorage.setItem('wwc_extra_catalog_v1:echo','{corrupt');},{chars,weapons,language});
   ...
   const pick=async(slot,id)=>{await page.locator(`#editor-echo [data-echo-action="pick"][data-value="${slot}"]`).click();await page.locator(`#echoPicker [data-echo-action="choose"][data-value="${id}"]`).click();};
   ...
   await open('Test 2');await page.locator('[data-echo-action="pick"]').first().click();assert.equal(await page.locator(`#echoPicker [data-value="${second.id}"]`).isDisabled(),true);await page.keyboard.press('Escape');await page.locator('#editorClose').click();
   ...
   await open();await page.locator('[data-echo-action="pick"][data-value="2"]').click();await page.locator('#echoPickerAdd').click();await form.locator('[name="echo"]').fill('Test Echo');await submit();await page.locator('#editorClose').click();assert.equal(await count(),2,'New draft copy is cancelled with character');
   await open();await page.locator('[data-echo-action="pick"][data-value="2"]').click();await page.locator('#echoPickerAdd').click();await form.locator('[name="echo"]').fill('Test Echo');await form.locator('[name="level"]').fill('0');await submit();await page.locator('#editorSave').click();assert.equal(await count(),3,'New draft copy commits with the character');
