...
const previous='7028d08236c863417972408265489cf933dbb0c3';
let latest=false;
const oldFiles=Object.fromEntries(['index.html','pwa.js','sw.js'].map(f=>[f,execFileSync('git',['show',`${previous}:${f}`],{cwd:root})]));
...
(async()=>{
 ...
 try{
  ...
  assert.equal(await p.locator('link[href="./layout.css"]').count(),0,'Old open tab must remain on old shell');
  ...
  await probe.goto(base);await probe.locator('link[href="./layout.css"]').waitFor({state:'attached'});
  ...
  assert.deepEqual(keys.filter(k=>k.startsWith('wuwa-companion-shell-')),['wuwa-companion-shell-057-pwa-7']);
