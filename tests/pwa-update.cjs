...
(async()=>{
 ...
 try{
  ...
  assert.equal(await p.evaluate(()=>typeof ConveneRules),'undefined','Old open tab must remain on old shell');
  const workers=context.serviceWorkers();
  let replacement;
  for(const worker of workers)if(await worker.evaluate(()=>CACHE_NAME).catch(()=>null)==='wuwa-companion-shell-057-pwa-17')replacement=worker;
  ...
  for(let attempt=0;attempt<100&&!activated;attempt++){
   activated=await replacement.evaluate(async()=>{
    ...
    return !self.registration.waiting&&self.registration.active?.state==='activated'&&keys.length===1&&keys[0]==='wuwa-companion-shell-057-pwa-17';
   });
  ...
  assert.deepEqual(keys.filter(k=>k.startsWith('wuwa-companion-shell-')),['wuwa-companion-shell-057-pwa-17']);
