...
function optimizationPage(){
 ...
 const allocation=allocatePlanCosts(plans,state.resources,progressionTables),filtered=ranked.filter(r=>gameSearch(r.character,optimizationQuery));
 ...
 companionPanel(tr('Améliorations proposées','Suggested improvements'),`<div class="companion-list">${filtered.map(r=>{
  ...
  return `<article class="optimization-card"><header><h3>${esc(gameLabel(c))}</h3><p>${r.active?tr('Objectif actif','Active goal'):r.favorite?tr('Équipe favorite','Favorite team'):r.teamCount?tr('Membre d’équipe','Team member'):tr('Réserve du compte','Account reserve')} · ${r.teamCount} ${tr('équipes','teams')}</p></header>${r.teams.length?`<p>${r.teams.map(t=>esc(t.name)).join(' · ')}</p>`:''}${unknown.length?`<p class="companion-note">${tr('À renseigner','To record')} : ${unknown.join(' · ')}</p>`:''}${tasks.length?`<ul>${tasks.slice(0,3).map(t=>`<li>${esc(t.text)}</li>`).join('')}</ul>`:`<p>${r.goal?tr('Les objectifs renseignés sont atteints.','Recorded targets are reached.'):tr('Aucun objectif : choisis les améliorations utiles à ton équipe.','No goal: choose upgrades useful to your team.')}</p>`}${plan?.missing.length?`<p class="companion-note">${tr('Coûts partiels : données à compléter.','Partial costs: data needs completing.')}</p>`:''}${plan?.allocation.length?`<details><summary>${tr('Répartition des ressources','Resource allocation')}</summary><div class="import-values">${plan.allocation.map(row=>`<p>${esc(materialName(row.id))} · ${tr('besoin','need')} ${row.need.toLocaleString(lang)} · ${tr('disponible après les priorités précédentes','available after earlier priorities')} ${row.reserved.toLocaleString(lang)} · ${tr('manque','missing')} ${row.missing??'?'}</p>`).join('')}</div></details>`:''}<div class="companion-actions">${companionButton('wish-plan',tr('Préparer l’objectif','Prepare goal'),c.id)}${companionButton('edit-plan-progress',tr('Renseigner la progression','Record progress'),c.id)}${companionButton('wish-profile',tr('Consulter la configuration','View build'),c.id)}</div></article>`;
