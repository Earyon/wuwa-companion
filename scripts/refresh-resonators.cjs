...
(async()=>{
 ...
 const [descriptions,tags]=await Promise.all(['BinData/skill/skilldescription.json','BinData/role/roletag.json'].map(read));
 const displayProperties=JSON.parse(fs.readFileSync(path.join(process.env.WUWA_SOURCE_CACHE,'CommonParam_RoleAttributeDisplay6.json'),'utf8'));
 assert.equal(displayProperties.length,6);
 ...
 for(const character of catalogue.characters){
  ...
  const layout=tree.map(n=>({id:n.Id,index:n.NodeIndex,parents:n.ParentNodes,skillId:n.SkillId,kind:n.NodeType,coordinate:n.Coordinate,properties:n.Property}));
  ...
  for(const language of ['en','fr']){
   ...
   locales[language]={Id:role.Id,Introduction:text(role.Introduction),Properties:displayProperties.map(id=>{const p=properties.find(p=>p.Id===id);assert.ok(p&&Number.isFinite(base[p.Key]));return {Name:text(p.Name),BaseValue:p.IsPercent?base[p.Key]/100+'%':base[p.Key]};}),Skills:kit.map(s=>{
    const node=tree.find(n=>n.SkillId===s.Id);
    const parameters=(s.SkillDetailNum||[]).map(x=>x.ArrayString?.join(' / ')??String(x));
    const SkillAttributes=descriptions.filter(d=>d.SkillLevelGroupId===s.SkillLevelGroupId).sort((a,b)=>a.Order-b.Order||a.Id-b.Id).map(d=>({Name:text(d.AttributeName),Values:Array.from({length:10},(_,i)=>{const values=d.SkillDetailNum.map(p=>p.ArrayString[Math.min(i,p.ArrayString.length-1)]);assert.ok(values.every(v=>typeof v==='string'));return d.Description?text(d.Description,values):values.join(' / ');})}));
    return {SkillId:s.Id,SkillType:clean(maps.en.get(`SkillType_${s.SkillType}_TypeName`)),TypeLabel:text(`SkillType_${s.SkillType}_TypeName`),SkillName:text(s.SkillName),SkillDescribe:text(s.SkillDescribe,parameters),SkillDescribeRich:richText(s.SkillDescribe,parameters),SkillAttributes,Icon:s.Icon,Consumes:node?.Consume?.length?[{Consume:node.Consume}]:[]};
  ...
  const base=bases.find(b=>b.Id===role.PropertyId);
  const data={schema:1,gameVersion:catalogue.gameVersion,sourceRevision:revision,localSource,Id:role.Id,layout,base:Object.fromEntries(properties.filter(p=>(p.IsShow||displayProperties.includes(p.Id))&&Number.isFinite(base[p.Key])).map(p=>[p.Id,base[p.Key]])),showProperties:displayProperties,tags:(role.Tag||[]).map(id=>{const tag=tags.find(t=>t.Id===id);assert.ok(tag);return {id,icon:tag.TagIcon,locales:Object.fromEntries(['fr','en'].map(language=>[language,{name:clean(maps[language].get(tag.TagName)),description:clean(maps[language].get(tag.TagDesc))}]))};}),locales};
